...
const Notifications = memo(function Notifications() {
  ...
  // Filter lives in local state because it is purely presentational.
  const [currentFilter, setCurrentFilter] = useState('all');
  // Memoized selector prevents recalculating filtered data unnecessarily.
  ...
  return (
    <>
      ...
      {loading ? (
        ...
        <div data-testid="notification-drawer" className={css(styles.notificationItems)} ref={DrawerRef}>
          {filteredNotifications.length > 0 ? (
            <>
              <p className={css(styles.p)}>Here is the list of notifications</p>
              <button type="button" className={css(styles.button)} onClick={handleToggleDrawer} aria-label="Close">
                <img src={closeIcon} alt="close icon" />
              </button>
              <div>
                {/* Clicking again on an active filter resets back to all. */}
                <button type="button" className='urgent' onClick={handleSetFilterUrgent}>‼️</button>
                <button type="button" className='default' onClick={handleSetFilterDefault}>??</button>
              </div>
